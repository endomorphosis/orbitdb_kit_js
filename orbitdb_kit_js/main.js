    ...
    async init(ctx) {
        ...
        if (!ctx){
            ...
            if (this.config.identityKey){
                this.ctx.config.identityKey = this.config.libp2p.identityKey;
            }
            else{
                this.ctx.config.identityKey = createFromPrivKey(deriveKeyPair(id))
            }
            ...
            if (this.config.libp2p.bootstrappers){
                this.ctx.bootstrappers = this.config.libp2p.bootstrappers
            }
            else{
                this.ctx.bootstrappers = []
            }
        }
        else{
            this.ctx = ctx;
            ...
            if (!this.ctx.bootstrappers){
                this.ctx.bootstrappers = this.config.bootstrappers
            }
        }
        ...
        let ipfs = await createHelia({blockstore: blockstore, libp2p: libp2p, datastore: datastore, blockBrokers: [bitswap()]})
        this.ipfs = ipfs
        ...
        let orbitdb = await createOrbitDB({ ipfs: ipfs, identity: identity, accessController: OrbitDBAccessController })
        this.orbitdb = orbitdb
        ...
        //     })
        // let oldHeads = await db.log.heads()
        return {orbitdb: orbitdb, db: db, oldHeads: oldHeads}
    }
